...
// 获取二维码key
export const getTwoCodeKey = () => http.get('/login/qr/key', {}, true);
// 生成二维码
export const createTwoCodeKey = (key: string) => http.get('/login/qr/create', { key, qrimg: true, }, true);
// 检测二维码是否过期
export const checkTwoCodeEffective = (key: string) => http.get('/login/qr/check', { key, }, true);
// 邮箱登录
...
// 登录状态
export const getStatus = () => http.get('/login/status', {}, true);
// 退出登录
export const logout = () => http.get('/logout', {}, true);
// 获取验证码
export const getCaptcha = (phone: string) => http.get('/captcha/sent', { phone, }, true);
// 校验手机是否已经注册过
export const checkPhoneExistence = (phone: string) => http.get('/cellphone/existence/check', {phone, }, true);
// 校验验证码
